    ...
    if filter_status == 'all':
        rdvs = db.execute("""
            SELECT r.id, r.date_rdv, r.heure, r.date_fin, r.heure_fin, r.etat,
                   ...
                   h.nom_hotel AS nom_hotel, h.personne_contact AS personne_contact
            FROM rendez_vous r
...
def serialize_rdv_event(rdv):
    ...
    return {
        ...
        'extendedProps': {
            ...
            'personne_contact': rdv['personne_contact'] if 'personne_contact' in rdv.keys() else '',
            'agent_name': f"{rdv['nom']} {rdv['postnom']} {rdv['prenom']}",
...
def api_rdvs():
    ...
    rdvs = db.execute("""
        SELECT r.id, r.date_rdv, r.heure, r.date_fin, r.heure_fin, r.etat,
               ...
               h.nom_hotel, h.personne_contact
        FROM rendez_vous r
        JOIN agent a ON r.agent_id = a.id
        JOIN hotel h ON r.hotel_id = h.id
    """).fetchall()
